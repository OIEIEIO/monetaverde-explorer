...
MempoolStatus::read_mempool()
{
    ...
    for (size_t i = 0; i < mempool_tx_info.size(); ++i)
    {
        ...
        double payed_for_kB = MCN_AMOUNT(_tx_info.fee) / tx_size;
        ...
        last_tx.fee_str          = xmreg::mcn_amount_to_str(_tx_info.fee, "{:0.4f}", false);
...
MempoolStatus::read_network_info()
{
    ...
    local_copy.hash_rate                  = (rpc_network_info.difficulty/60);
...
bf::path MempoolStatus::blockchain_path {"/home/monetaverde /.monetaverde/lmdb"};
string MempoolStatus::deamon_url {"http:://127.0.0.1:36081"};
